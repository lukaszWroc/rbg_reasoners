...
private:
  ...
  bool check();
  bool check(uint64_t end, uint64_t start);
  bool checkKinghts(uint64_t end, uint64_t start);
  bool checkPawns(uint64_t king, uint64_t otherPawns);
  bool checkKing(uint64_t king, uint64_t otherKing);
  bool checkOther(uint64_t king, uint64_t otherQueens, uint64_t otherRooks,
    uint64_t otherBishops, uint64_t freeSpace);
